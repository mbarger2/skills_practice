...
where date_format(rental_date, "%Y") = (select date_format(max(rental_date), "%Y") from rental)
and date_format(rental_date, "%M") = (select date_format(max(rental_date), "%M") from rental);
...
select *, as time_of_week from rental,