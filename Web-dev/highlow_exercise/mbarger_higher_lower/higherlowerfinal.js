...
function create_max(){
    ...
    if(newmax > 1){
        ...
        message1.innerHTML = `You have chosen ${goodmax}.<br><br>I have chosen a number from 1 to ${goodmax}. Please guess my number.`;
...
function do_guess(){
    ...
    if((guess==num) && (guess_array.length > 0) && (flag == 'white')){
    ...
    else if((guess==num) && (guess_array.length == 0) && (flag == 'white')){
        message2.innerHTML =`You got it on the first try, way to go!`;
    }
    else if(guess>goodmax || guess<1 ){
        message2.innerHTML = `That guess is out of range. Please guess a number between 1 and ${goodmax}.`;
    }
    else if((guess > num) && (flag == 'white')){
